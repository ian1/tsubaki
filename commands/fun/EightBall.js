...
class EightBall extends Command {
  constructor() {
    super('8ball', 'Will answer your burning (yes or no) questions.', ' <question>');
  }
  ...
  executeInternal(message, args, bot, db) {
    ...
    if (question == '' || question === undefined) return message.channel.sendTemp(Tsubaki.Style.warn(':eye_in_speech_bubble: _Give me a question!_'), 10000);
    else {
      message.channel.send(':thinking: ' + Tsubaki.Style.italicize(thinking[Math.floor(Math.random() * thinking.length)])).then(msg => {
        ...
        setTimeout(function () {
          ...
          msg.editTemp(Tsubaki.Style.bold(balls[Math.floor(Math.random() * balls.length)]), 20000);
        }, 1000);
      });
    }
  }
